...
class MenuController extends Controller
{
    ...
    public function prosesUbah(Request $request){
        ...
        $menu->nama_menu = $request->nama_menu;
        $menu->jenis_menu = $request->jenis_menu;
        $menu->url_menu = $request->link_url;
        $menu->target_menu = $request->target_menu;
        $menu->parent_menu = $request->link_page;
    ...
    private function getUrutanMenu($parent = null){
        if ($parent == null){
            ...
            $noUrutSubMenu = null;
            $noUrutSubMenu = Menu::select('urutan_menu')
                ->whereNotNull('parent_menu')
                ->where('parent_menu', '=', $parent)
                ->orderBy('urutan_menu', 'desc')
                ->first();
            if ($noUrutSubMenu == null){
                $noUrutSubMenu = 1;
            }else{
                $noUrutSubMenu = $noUrutSubMenu->urutan_menu - 1;
            }
